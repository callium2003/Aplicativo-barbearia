...
| 2026-08-06 | Aplicação das migrations de comissão no Supabase remoto de homologação | As três migrations de comissão (`20260804050000`, `20260804060000`, `20260804070000`) constam como aplicadas no Supabase remoto de homologação `irszgnkzqseljowckrgz` (registradas no relatório de auditoria `docs/AUDIT-PROFESSIONAL-COMMISSION-SECURITY.md`). A homologação funcional visual da proprietária permanece pendente. | Aplicada em homologação (conforme relatório de auditoria); homologação da proprietária pendente |
| 2026-08-06 | Revogação explícita de `EXECUTE` do papel `anon` nas RPCs de comissão | RPCs financeiras públicas devem revogar explicitamente `EXECUTE` de `anon` além de `PUBLIC`. A migration `20260806050000_revoke_anon_commission_rpc_execute.sql` revogou `EXECUTE` de `anon`, mantendo permissão somente para `authenticated` e eliminando o alerta do Supabase Security Advisor (registrada em `docs/AUDIT-PROFESSIONAL-COMMISSION-SECURITY.md`). | Aplicada em homologação (conforme relatório de auditoria) |
| 2026-08-06 | Congelar alterações remotas até reconciliar migrations | Não executar `db push`, `migration repair` ou SQLs mutáveis até produzir e aprovar o mapa formal de reconciliação entre arquivos locais e histórico remoto. | Aprovada |
| 2026-08-06 | Trabalhar em etapas pequenas e ordenadas | Executar a estabilização técnica na ordem estrita de etapas (0 a 9) documentada em `AUDIT-REMEDIATION-PLAN-2026-08-06.md`. | Aprovada |
| 2026-08-06 | Corrigir segurança antes de novas funcionalidades | Travar o desenvolvimento de novas funcionalidades de produto até concluir a reconciliação, correção de convites/tokens, Storage RLS, permissões de manager e CI. | Aprovada |
| 2026-08-06 | Não usar policy ampla para solucionar erros de permissão | Proibir o uso de políticas permissivas genéricas (ex: `USING (true)`) para contornar falhas de permissão; usar RLS estrita por tenant e RPCs com escopo limitado. | Aprovada |
| 2026-08-06 | Exigir evidência e teste antes de concluir item | Qualquer item documentado como concluído deve apresentar logs de execução, testes automatizados e evidências empíricas de sucesso. | Aprovada |
