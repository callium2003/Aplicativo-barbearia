...
| 2026-08-04 | Endurecimento de segurança e isolamento total da comissão | A coluna `commission_rate_percent` foi removida de `public.professionals` e migrada para a tabela privada `public.professional_commission_settings`. A tabela financeira teve todo acesso direto revogado (`anon`, `authenticated`, `PUBLIC`). Leitura e escrita ocorrem de forma estrita via RPCs `SECURITY DEFINER` e validação com `auth.uid()`, resolvendo o tenant no banco, impedindo que gerentes alterem diretamente dados de `professionals`. Bloqueio transacional `FOR UPDATE` e testes simultâneos concorrentes protegem contra race conditions, registrando log real de auditoria. Validação decimal no frontend e no banco rejeitam anomalias (textos, negativos, >100, ou frações extras). Teste SQL e testes de interface estáticos validados. | Implementada |
