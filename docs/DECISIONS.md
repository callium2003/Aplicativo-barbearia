...
| 2026-08-08 | Worker de e-mail usa Supabase Edge Function + `pg_cron` + `pg_net` + Vault + Resend | Cron chama `process-notifications` a cada minuto; segredos ficam no Vault e a função valida segredo próprio antes das operações privilegiadas. | Implementada/homologada |
...
| 2026-08-08 | Objetos operacionais do worker devem ser reproduzíveis no Git sem versionar valores secretos | Edge Function foi colocada em `supabase/functions/process-notifications/`; a migration `20260808183718_version_notification_worker_runtime.sql` versiona extensões, helper/grants e configuração do Cron. URL/chaves/segredo permanecem apenas no Vault por ambiente. | Implementada |
| 2026-08-08 | Cron não deve ter project ref hardcoded na migration | O job lê `barbeariasp_project_url` e `barbeariasp_notification_cron_secret` do Vault por nome; `private.configure_notification_worker_cron()` recria o job após provisionamento. | Implementada |
| 2026-08-08 | Dependência da Edge Function deve ser fixada | `@supabase/supabase-js@2.97.0` foi fixado para evitar mudança silenciosa do runtime. | Implementada |
