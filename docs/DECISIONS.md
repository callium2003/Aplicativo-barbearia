...
| 2026-08-06 | Revogação explícita de `EXECUTE` do papel `anon` nas RPCs de comissão | RPCs financeiras públicas devem revogar explicitamente `EXECUTE` de `anon` além de `PUBLIC`, porque grants diretos persistem no ambiente Supabase quando criados no schema `public`. A migration `20260806050000_revoke_anon_commission_rpc_execute.sql` revogou `EXECUTE` de `anon`, mantendo permissão somente para `authenticated` e eliminando o alerta do Supabase Security Advisor. | Aplicada em homologação e validada tecnicamente |
| 2026-08-07 | Remover Drizzle/D1 após análise coordenada de Worker, build e dependências | Foi comprovado que D1 não tinha binding ativo nem uso no runtime e que o schema Drizzle era legado do template. `drizzle-orm`, `drizzle-kit`, adaptador D1, exemplos, configuração e a cadeia depreciada `@esbuild-kit`/`esbuild 0.18.20` foram removidos. Supabase/PostgreSQL passa a ser explicitamente o único banco funcional. A remoção foi validada com `npm ci`, typecheck, testes, lint e build limpos. | Implementada e validada |
