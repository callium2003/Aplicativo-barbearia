...
| Área do Cliente / §§12–13, 25 | Agenda, histórico, perfil, privacidade e navegação inferior contextual implementados; cabeçalho não duplica “Meu perfil”; página pública mostra navegação de Cliente somente para sessão exclusivamente de Cliente | Navegação e fluxos principais já homologados pelo responsável; exportação, protocolos e encerramento possuem testes específicos registrados no código | **Construído/homologado no escopo já confirmado.** Não reabrir sem regressão concreta |
