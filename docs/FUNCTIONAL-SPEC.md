...
**IMPLEMENTADO** significa que o código e a validação local existem. **IMPLEMENTADO, COM CORREÇÃO DE SEGURANÇA PENDENTE** indica recurso entregue porém com ressalva de auditoria. **PARCIAL** significa que há lacuna funcional, operacional ou de homologação. **PLANEJADO** ainda não foi entregue.
...
- **IMPLEMENTADO, COM CORREÇÃO DE SEGURANÇA PENDENTE:** foto pública da barbearia. Owner e manager podem selecionar JPG, PNG ou WebP de até 3 MB; há prévia e otimização. *Ressalva:* A exclusão da foto antiga no Storage falha devido a erro de qualificação na RLS de `DELETE`.
...
- **IMPLEMENTADO EM HOMOLOGAÇÃO:** percentual de comissão por profissional (`0%` a `100%`) configurado por owner e manager em `/painel/configurar` e persistido via RPCs seguras `get_professional_commission_rates` e `set_professional_commission_rate` com auditoria em `audit_logs` e privilégio `EXECUTE` revogado de `anon` (consta como aplicado no Supabase remoto de homologação conforme `docs/AUDIT-PROFESSIONAL-COMMISSION-SECURITY.md`; estando pendentes a homologação funcional da proprietária e os testes de CI). Barbeiros, clientes, anônimos e usuários sem vínculo são bloqueados.
...
## Navegação da gestão e convites
...
- **IMPLEMENTADO, COM CORREÇÃO DE SEGURANÇA PENDENTE:** convites de equipe (`team_invitations`). Owner pode convidar gerente (`manager`) ou barbeiro (`barber`); manager pode convidar barbeiro (`barber`). O sistema gera um link individual com token único. *Ressalvas:* O token bruto é repassado na URL/`redirectTo`, as RPCs administrativas de convite possuem permissão de execução para `anon`, e `get_invitation_details` expõe e-mail completo ao anônimo.
