...
- A versão atualmente hospedada foi publicada em 16/09/2026; uma nova publicação continua exigindo autorização explícita e verificação própria.
...
| Área do Cliente / §§12–13, 25 | Agenda, histórico, perfil, privacidade e navegação inferior contextual implementados; cabeçalho não duplica “Meu perfil”. Em 16/09/2026, os atalhos redundantes Meus dados, Preferências de comunicação e Privacidade e meus dados foram removidos da agenda; essas funções permanecem concentradas em Meu perfil. A página pública mostra navegação de Cliente somente para sessão exclusivamente de Cliente | Navegação e fluxos principais já homologados pelo responsável; remoção coberta por teste de regressão de composição. Exportação, protocolos e encerramento possuem testes específicos registrados no código | **Construído; atualização visual local pendente de homologação do responsável.** Não altera dados, preferências, privacidade ou rotas |
...
| Segurança e Data API / §§25.6, 28, 30 | Exposição automática de novas tabelas foi desativada; migrations recentes reforçaram RLS, grants e funções de Storage. Tabelas internas sem policy permanecem em negação por padrão: sem DML para `anon`/`authenticated`. Das 52 funções `SECURITY DEFINER`, nenhuma é executável por `anon`, todas fixam `search_path` e as 32 para `authenticated` compõem a API autenticada com validações internas | Auditorias SEG-03–SEG-05 e reconciliação SEG-09 verificaram o remoto; não foi identificado bypass anônimo ou de tenant que exija nova migration. Proteção contra senhas vazadas não existe no plano Free | **Reconciliado.** Alertas genéricos do Advisor não são pendências automáticas |
...
| Privacidade e exportação / §25 | Direitos do Cliente, exportação, protocolos, consentimentos e encerramento possuem implementação e proteções de identidade; exportação operacional da barbearia está integrada ao Supabase remoto e à aplicação hospedada | RPC owner-only homologada por transação com `ROLLBACK`; interface publicada em `/painel/assinatura/dados`. Homologações de privacidade anteriores são preservadas; revisão jurídica continua decisão externa | **Construído, integrado e publicado.** Jurídico somente quando o responsável escolher |
| Qualidade e migrations / §36 | Teste de linhagem inclui as migrations recentes de proteção pública, equipe, performance e retenção; migrations históricas não foram renomeadas ou reparadas. O script Windows de empacotamento passou a iniciar `npx.cmd` via `ComSpec`, compatível com Node 24 local sem alterar o Node 22 remoto | Rodada ampla final: 212/212 testes, typecheck e lint aprovados; build local Hostinger com 36 rotas; `git diff --check` sem erros, apenas avisos LF/CRLF | **Baseline local validada para a versão publicada.** Reexecutar validações proporcionais após novas mudanças |
| Publicação / §§30, 34–35 | Versão completa do checkout local publicada em `barbeariasp.cullentech.com.br` pelo build Hostinger `01a0a7f4-cef0-7070-a8c7-45aa0cf39219`, em Node 22 | Build remoto concluído, 390 pacotes auditados sem vulnerabilidades conhecidas, 36 rotas geradas; cache limpo; landing, `/api/health`, `/painel/assinatura/dados` e `/painel/acesso-e-seguranca` responderam HTTP 200, com marca real e health `ok` | **Publicado em 16/09/2026.** A publicação não cria commit ou push e não substitui homologação humana de todos os fluxos autenticados |
