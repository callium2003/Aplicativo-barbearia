...
- **IMPLEMENTADO:** login por Google e magic link. E-mail usa `signInWithOtp`, mostra sucesso ou erro e retorna a `/painel`; Google usa `signInWithOAuth` com o mesmo destino.
...
- **IMPLEMENTADO:** criação com nome/slug, página por slug, serviços, disponibilidade, agendamento, profissionais, horários, pausas e bloqueios.
...
- **IMPLEMENTADO:** cliente global autenticado, relação isolada por barbearia, histórico real por tenant e lista de clientes. Owner e manager podem iniciar conversa manual de WhatsApp com o cliente.
