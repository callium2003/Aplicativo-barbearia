...
O bloqueio operacional do worker e do monitor foi removido. A migration incremental também foi aplicada e validada: o limite de 300 segundos foi recusado, um nonce novo foi aceito uma vez, a repetição foi recusada e os registros da prova foram removidos; o Cron respondeu HTTP 200 depois da mudança.

Em 06/09/2026, os dois achados médios do diff scan foram corrigidos no Supabase. `delete-my-customer-account` v6 passou a remover os objetos pela Storage API antes da anonimização; as migrations remotas `20260906124420` e `20260906124513` expõem somente o manifesto do próprio usuário e removem a exclusão direta de `storage.objects`. `process-notifications` e `monitor-platform-health` v18 validam o segredo da Edge Function antes de abrir a conexão Postgres. Chamadas inválidas retornaram HTTP 401 e não alcançaram o banco nos testes; chamadas controladas legítimas retornaram HTTP 200. A Hostinger não foi publicada neste ciclo.
