...
Atualizado em **12/08/2026**. Este arquivo organiza os relatos de uso e as pendências técnicas. Ele não autoriza, por si só, alterações no Supabase, Hostinger, dados ou produção.
...
| QA-02 | Revalidar login por magic link no domínio. | Auth | Pendente | Fluxo implementado; roteiro detalhado com conta de cliente, evidência segura e resultado esperado disponível em `ROTEIRO-HOMOLOGACAO-ATUAL.md`. Falta teste real pós-publicação. |
| QA-03 | Testar fluxo completo do cliente: página pública, seleção, login, confirmação, cancelar e reagendar. | Agendamento | Pendente | Fluxo implementado; roteiro detalhado com evidência de cada etapa disponível em `ROTEIRO-HOMOLOGACAO-ATUAL.md`. Falta execução com conta real. |
...
| QA-12 | Atualizar os documentos de status da Hostinger após a correção do loop e desligamento do cacheless. | Documentação | Concluído | Registros atualizados até o deploy `63c0198` (build `019ff458-d0c9-731c-889c-028b63132059`); em 12/08/2026 a checagem versionada de saúde confirmou HTTP 200 com `cache-control: no-store` e o cache normal foi confirmado por cabeçalho dinâmico. |
...
| PR-03 | Revisar avisos do Supabase Advisor individualmente, sem abrir permissões apenas para eliminar alertas. | Segurança Supabase | Em validação | Advisor reexecutado em 11/08/2026. Três tabelas com RLS sem policy permanecem fechadas por padrão; as RPCs SECURITY DEFINER listadas são intencionais e possuem validação interna já coberta pela auditoria. Os índices sem uso são apenas informativos em ambiente de baixo tráfego; não serão removidos sem métricas de produção. |
| PR-04 | Manter registrado o risco de proteção contra senhas vazadas desativada até haver upgrade de plano. | Auth Supabase | Pendente | O Advisor confirmou em 11/08/2026 que a proteção contra senhas vazadas continua desativada. Permanece limitação do plano; não há correção segura a aplicar somente no código do projeto. |
...
| PR-06 | Adicionar monitoramento de disponibilidade, erros de login, agenda, e-mail, imagens e filas. | Observabilidade | Em validação | Em 12/08/2026 foi configurado e validado o monitor de disponibilidade da aplicação: Supabase Cron a cada 7 minutos, função autenticada por segredo no Vault e e-mail somente em queda/recuperação para o responsável técnico e barbearias ativas. A primeira execução normal confirmou estado saudável, sem erro e sem alerta indevido; o endpoint `/api/health` não expõe dados. Falta definir uma solução realmente independente para falhas totais do Supabase/e-mail, além de telemetria específica de login, agenda e imagens. |
