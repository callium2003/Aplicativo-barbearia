...
| PR-06 | Adicionar monitoramento de disponibilidade, erros de login, agenda, e-mail, imagens e filas. | Observabilidade | Em validação | Endpoint público mínimo `/api/health` foi publicado e respondeu HTTP 200 com `no-store`, sem expor dados. Fila e entregas de notificação já possuem monitor interno; ainda falta definir serviço externo e alertas para erros de login, agenda e imagens. |
