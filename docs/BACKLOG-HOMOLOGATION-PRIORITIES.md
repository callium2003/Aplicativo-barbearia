...
| --- | --- | --- | --- | --- |
| UX-01 | Bloquear que dono, gestor e profissional façam agendamento na própria barbearia usando a conta administrativa. Permitir somente com conta de cliente separada. O bloqueio deve existir na interface e no banco. | Login, página pública e agendamento | Em validação | Interface e regra no Supabase atualizadas; RPC verificada sem EXECUTE anônimo e com bloqueio de perfil administrativo. Falta homologação com contas reais. |
...
| UX-03 | Impedir que cliente encontre uma tela administrativa que pede login novamente. A rota não deve ser oferecida a esse perfil. | Navegação e permissões | Em validação | Atalho administrativo removido da navegação do cliente; falta roteiro em navegador autenticado. |
...
| UX-06 | Reduzir o card de “Próximos atendimentos” e alinhar seu tamanho ao card de histórico. | Área do cliente | Em validação | Card grande substituído pelo mesmo padrão compacto dos itens de agenda; falta inspeção em celular real. |
...
| UX-15 | Corrigir menus por perfil em todas as telas: cliente, dono/gestor e profissional devem ver apenas as opções permitidas. | Navegação | Pendente | Relato funcional e visual do usuário. |
| UX-16 | Remover “Abrir painel de gestão” de todas as telas administrativas; ele está sobrepondo a navegação e o sino em navegador e celular. Preservar “Sair”. | Painel administrativo | Em validação | Atalho removido e botão de saída deslocado para não cobrir o cabeçalho; teste automatizado confirma remoção. |
...
| UX-19 | Ajustar o texto do consentimento de promoções para “Não quero receber promoções e novidades da barbearia”, opcional e desmarcado por padrão. | Consentimentos | Pendente | Texto solicitado pelo usuário. |
...
| UX-21 | Revisar navegação do computador para disponibilizar as ações importantes sem copiar literalmente o menu inferior do celular. | Navegação desktop | Pendente | Relato visual do usuário. |
| UX-22 | Revisar menu da página inicial: no computador há “Como funciona”, “Planos”, “Entrar” e teste; no celular a navegação é diferente. | Página inicial | Em validação | Navegação mobile agora preserva os mesmos atalhos, com texto e espaçamento compactos. |
| UX-23 | Revisar visão da agenda do profissional por período e filtros de status, sem exigir uma data para descobrir atendimentos. | Agenda profissional | Pendente | Pendência funcional já registrada. |
| UX-24 | Melhorar edição de profissional e comissão deixando inequívoco o nome do profissional em edição. | Configurações | Pendente | Relato de usabilidade do usuário. |
| UX-25 | Melhorar visual e entendimento de horários, pausas, intervalos e ausências; “intervalo semanal” deve representar corretamente dia e horário. | Configurações de agenda | Pendente | Relato de usabilidade do usuário. |
| UX-26 | Permitir que o profissional ajuste apenas a própria disponibilidade, pausas e ausências dentro de suas permissões. | Área do profissional | Em validação | Função existe parcialmente; falta homologação por perfil. |
...
| ID | Ajuste | Área | Status | Evidência disponível |
| --- | --- | --- | --- | --- |
| QA-01 | Revalidar login Google no domínio após a correção do loop. | Auth | Em validação | Loop corrigido e publicado no commit `c74fa2b`; falta teste real de usuário. |
...
| QA-05 | Reproduzir e corrigir erro de upload de foto da barbearia (`new row violates row-level security policy`). | Supabase Storage | Pendente | Erro relatado anteriormente. |
...
| PR-08 | Fazer revisão final de RLS, Storage, sessão, Auth, URLs de redirecionamento e isolamento de tenant. | Segurança | Pendente | Requisito antes de produção. |
