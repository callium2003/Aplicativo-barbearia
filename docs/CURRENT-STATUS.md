...
- Publicacao Next/Node na Hostinger atualizada pelo build `01a0773d-5881-709e-aafa-1a7daf78e78d`, concluído em 06/09/2026 com Node 22, npm, `build` e saída `.next`. A raiz `/` respondeu HTTP 200 com o conteúdo da landing atual após a limpeza de cache; `/entrar` também respondeu HTTP 200. O pacote contém apenas configuração pública do Supabase, sem segredos administrativos.
- Disponibilidade: `/api/health` respondeu HTTP 200 com `status: ok` após o build de 06/09/2026. A rota comprova resposta do Next, não a saúde do banco ou o funcionamento dos alertas.
