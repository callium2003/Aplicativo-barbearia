...
- **Profissional (`public.professionals`)**: Entidade operacional da agenda. Não contém o percentual de comissão. A comissão é armazenada na tabela privada `public.professional_commission_settings`, vinculada por `professional_id`. Owner e manager acessam os dados de comissão exclusivamente pelas RPCs `get_professional_commission_rates` e `set_professional_commission_rate` com `SECURITY DEFINER` e auditoria em `audit_logs`. O tenant é derivado do profissional no banco de dados. O acesso direto à tabela financeira por roles do navegador (`anon`, `authenticated`, `PUBLIC`) é revogado. Barbeiro, cliente, anônimo e usuário sem vínculo não acessam comissão. Pode receber agendamentos sem ter conta de acesso ou login no Supabase Auth.
