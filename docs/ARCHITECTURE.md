...
- **Profissional (`public.professionals`)**: Entidade operacional da agenda. Não contém a coluna `commission_rate_percent`. A comissão é armazenada na tabela privada `public.professional_commission_settings`, vinculada exclusivamente por `professional_id`. Owner e manager acessam os dados de comissão pelas RPCs `get_professional_commission_rates` e `set_professional_commission_rate` com `SECURITY DEFINER`, `search_path TO ''` e auditoria em `audit_logs`. O privilégio `EXECUTE` foi revogado do papel `anon`, pertencendo a `authenticated`. *Inconsistência RLS:* Os logs registraram violação de RLS na tabela `professionals`. Por desenho, `professionals` não possui `user_id`; a associação ocorre por `team_members.user_id` e `team_members.professional_id`. Sem a instrução SQL nos logs, a hipótese mais provável é uma tentativa de escrita realizada por uma sessão que não possuía o papel de proprietário (`owner`), visto que as políticas atuais de escrita direta autorizam apenas o proprietário.
