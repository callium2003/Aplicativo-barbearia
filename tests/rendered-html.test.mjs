...
test("keeps the public booking flow connected to required data and consent operations", async () => {
  ...
  assert.match(publicPage, /"book_customer_appointment"/);
  assert.match(publicPage, /"reschedule_customer_appointment"/);
  ...
  const bookingPayload = publicPage.match(/const bookingPayload = \{([\s\S]*?)\};/);
