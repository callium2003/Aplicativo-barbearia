...
import { formatRecurringBreakLabel, weekdayLabels } from "@/utils/format-recurring-break";
      ...
      {!professionalId ? <div className="product-card product-empty" style={{ marginTop: 18 }}>Selecione um profissional para editar a disponibilidade.</div> : <div className="product-grid cols-2" style={{ marginTop: 18 }}>
        <form className="product-card pad" onSubmit={save}>
          ...
          <div style={{ display: "grid", gap: 14, marginTop: 20 }}>
            <div className="product-field"><label>Dia</label><select className="product-select" name="weekday">{weekdayLabels.map((day, index) => <option key={day} value={index}>{day}</option>)}</select></div>
          ...
          <div className="product-chip-row" style={{ marginTop: 20 }}>{breaks.map((item) => <span className="product-chip" key={item.id}>{formatRecurringBreakLabel(item.weekday, item.starts_at, item.ends_at)}</span>)}{!breaks.length && <small style={{ color: "#777" }}>Nenhuma pausa recorrente cadastrada.</small>}</div>
        </form>
