...
export default function Home() {
  return <main>
    <nav><div className="brand"><span>BARBEARIA</span>SP</div><div className="navlinks"><a href="#como-funciona">Como funciona</a><a href="#planos">Planos</a><a href="/entrar">Entrar</a><a className="button small" href="/entrar">Começar teste grátis</a></div></nav>
    <section className="hero">
      <div className="hero-copy"><p className="eyebrow">AGENDE. ORGANIZE. CRESÇA.</p><h1>Sua barbearia<br/><em>em um só link.</em></h1><p className="lead">Crie a página da sua barbearia, mostre seus serviços e deixe a agenda organizada para você e seus profissionais.</p><div className="actions"><a className="button" href="/entrar">Teste grátis por 30 dias</a><a className="text-link" href="#planos">Ver planos →</a></div><p className="note">Sem cartão no cadastro. Sem aplicativo para o cliente baixar.</p></div>
    ...
    <section id="planos" className="plans"><p className="eyebrow">PLANOS E TESTE GRATUITO</p><h2>Experimente antes<br/>de contratar.</h2><p className="plans-intro">Toda barbearia nova recebe 30 dias gratuitos com acesso completo. Ao fim do teste, escolha o período que fizer mais sentido para seu negócio.</p><div className="trial-card"><div><b>30 DIAS GRÁTIS</b><h3>Comece agora, sem cartão.</h3><p>Configure sua página, serviços, profissionais e agenda durante o período de teste.</p></div><a className="button" href="/entrar">Começar teste grátis</a></div><div className="plans-grid">{plans.map(([name, description, billing])=><article key={name} className="plan-card"><p>{name.toUpperCase()}</p><h3>{name}</h3><strong>Valor a definir</strong><span>{description}</span><small>{billing}</small><a href="/entrar">Quero este plano</a></article>)}</div><p className="plans-note">Os valores serão informados antes da contratação. O pagamento será processado de forma segura.</p></section>
    <section id="acesso" className="cta"><div><p className="eyebrow">PRIMEIRA VERSÃO</p><h2>Coloque sua barbearia no ar.</h2><p>Comece criando seu espaço. Depois, você personaliza tudo: marca, equipe, preços e agenda.</p></div><a className="button light" href="/entrar">Criar minha página</a></section>
