-- A customer account closure must erase the relationship, not the operational
-- chronology. Completed, cancelled and no-show appointments remain available
-- to the barbershop only as anonymous snapshots for reports and commissions.
...
begin
  ...
  v_request := private.open_customer_privacy_request(
    v_customer_id,
    'ACCOUNT_DELETION',
    jsonb_build_object('source', 'customer_portal', 'anonymization_version', '4')
  );
  ...
  -- An active future booking cannot survive without an identifiable customer.
  -- All past and non-active records are kept and anonymized below.
