revoke all on function public.save_my_customer_marketing_preferences(uuid, boolean, boolean) from public, anon, authenticated;
drop function if exists public.save_my_customer_marketing_preferences(uuid, boolean, boolean);
...
begin
  ...
  v_request := private.open_customer_privacy_request(
    v_customer_id,
    'ACCOUNT_DELETION',
    jsonb_build_object('source', 'customer_portal', 'anonymization_version', '2')
  );
